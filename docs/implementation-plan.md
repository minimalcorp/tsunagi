...
| Phase 4  | Claude Agent SDK統合      | 2-3日 | 高     |
...
```bash
npm install simple-git @anthropic-ai/agent-sdk @hello-pangea/dnd
```
...
Claude Agent SDKを統合し、タスクに対してプロンプトを実行できるようにする。

### タスク

#### 4.1 Claude Agent Client実装
...
**重要**: REST APIとWebSocketの**デュアルインターフェース**を実装し、WebSocket接続失敗時にREST APIポーリングへフォールバックできるようにする。サーバー側のcontroller層で状態管理を統一し、両方のインターフェースから同じロジックを呼び出す設計とする。
...
**ファイル**: `src/lib/task-controller.ts`

- [ ] タスク操作のcontroller層を実装
- [ ] REST APIとWebSocketの両方から呼び出せる共通ロジック
- [ ] イベント発行機能（REST/WebSocket両対応）

#### 5.3 WebSocket Server実装
...
**既存APIファイル**: `src/app/api/tasks/*/route.ts`

- [ ] Controller層の利用に変更
- [ ] WebSocket接続時もREST APIが動作することを保証
...
- [ ] タスク状態の自動更新
- [ ] WebSocket接続失敗検知
- [ ] REST APIポーリングへの自動フォールバック
- [ ] WebSocket再接続時のポーリング停止

### 完了条件

- [ ] WebSocket接続が確立される
- [ ] タスク変更が全クライアントに通知される
- [ ] WebSocket接続失敗時にREST APIポーリングにフォールバックする
- [ ] WebSocket再接続時にポーリングが停止する
- [ ] REST APIとWebSocketの両方で同じcontroller層を使用している
- [ ] 自動再接続が動作する

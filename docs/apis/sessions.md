...
| パラメータ | 型                                                                                          | 必須 | 説明                 |
| ---------- | ------------------------------------------------------------------------------------------- | ---- | -------------------- |
| status     | 'running' \| 'waiting_for_permission' \| 'paused' \| 'completed' \| 'failed' \| 'cancelled' | ✗    | ステータスでフィルタ |
...
```bash
GET /api/sessions?status=waiting_for_permission
```
...
**使用タイミング**: セッションが `waiting_for_permission` 状態の時

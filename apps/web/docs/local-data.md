...
cd ~/.tsunagi/workspaces/{owner}/{repo}/.bare
...
```
/Users/username/.tsunagi/workspaces/minimalcorp/tsunagi/.bare         (bare)
/Users/username/.tsunagi/workspaces/minimalcorp/tsunagi/main    abc1234 [main]
/Users/username/.tsunagi/workspaces/minimalcorp/tsunagi/feat-auth  def5678 [feat/auth]
```
...
並列アクセスによるレースコンディションを防ぐため、Repository層とQueue機構を実装します。

#### 設計方針

1. **Repository層**: JSONファイルへの全アクセスをRepository経由にする
2. **Queue機構**: 書き込み操作を順序制御し、JSONファイルの破損を防ぐ
3. **副作用の分離**: upsert/deleteなど副作用のある操作をRepository層で定義
4. **並列呼び出し対応**: 複数の操作が同時に呼ばれてもQueue経由で順次実行
...
interface TaskRepository {
  ...
  updateClaudeState(id: string, state: ClaudeState): Promise<Task>;
}
```

#### Queue実装

各JSONファイルごとに独立したqueueを保持：
...
- **RepoRepository**: `upsertRepo()`, `deleteRepo()`
- **SessionRepository**: `upsertSession()`, `deleteSession()`

#### 使用例

```typescript
// Repository層のインスタンス（シングルトン）
const taskRepo = new TaskRepository('~/.tsunagi/state/tasks.json');

// 並列で呼ばれてもqueueで順次実行される
await Promise.all([
  taskRepo.upsertTask(task1),
  taskRepo.upsertTask(task2),
  taskRepo.deleteTask('id-3'),
]);

// 読み取りは並列実行可能
const [task1, task2] = await Promise.all([taskRepo.getTask('id-1'), taskRepo.getTask('id-2')]);
```
    ...
    case '0.0':
      ...
      return {
        ...
        tasks: data.map((task: any) => ({
          ...task,
          worktreeStatus: task.worktreeStatus || 'created', // 既存タスクは作成済みと仮定

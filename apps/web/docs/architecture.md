...
- **@hello-pangea/dnd** - ドラッグ&ドロップ機能（タスクの並び替え用）
...
- **WebSocket (Socket.IO)** - リアルタイム通信（Phase 5）
  - デュアルインターフェース設計: REST API + WebSocket
  - フォールバック戦略: WebSocket失敗時はREST APIポーリング
  - Controller層による統一的な状態管理

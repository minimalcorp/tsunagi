...
Claude API KeyまたはOAuth Tokenは、各スコープで**1つのみ**設定可能です。

### トークンの種類

- **ANTHROPIC_API_KEY**: 接頭辞 `sk-ant-api` で始まるAPI Key
- **CLAUDE_CODE_OAUTH_TOKEN**: 接頭辞 `sk-ant-oat` で始まるOAuth Token

### 自動検出

設定画面でトークン値を入力すると、接頭辞から自動的にキーが判定されます：

```
入力値: sk-ant-api03-xxxxx
→ 自動検出: ANTHROPIC_API_KEY

入力値: sk-ant-oat01-xxxxx
→ 自動検出: CLAUDE_CODE_OAUTH_TOKEN
```

### 制約

- 各スコープ（Global/Owner/Repo）で**どちらか1つ**のみ設定可能
- 既にトークンが設定されている場合、追加ではなく編集のみ可能
- トークンタイプを変更する場合（API Key ⇔ OAuth Token）、編集で新しいトークンを入力すると自動的に置き換わる
...
# Owner (minimalcorp): OAuth Token
CLAUDE_CODE_OAUTH_TOKEN=sk-ant-oat01-xxxxx

# Repo (minimalcorp/tsunagi): API Key (プロジェクト固有)
ANTHROPIC_API_KEY=sk-ant-api03-yyyyy
```

優先順位に従い、`minimalcorp/tsunagi`では最後のAPI Keyが使用されます。
...
環境変数設定画面のUI仕様については [docs/pages/environment-settings.md](./pages/environment-settings.md) を参照してください。

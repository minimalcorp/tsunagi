...
音声入力の文字起こし結果をローカルLLMで整形するためのサーバー。mlx-lm (Apple Silicon GPU) で応答を生成する。
...
`mlx_lm.server`が提供するOpenAI互換の`/v1/chat/completions`をそのまま公開する。

起動している間、tsunagiのSettingsでローカルLLMを有効にすると、音声入力の文字起こし結果がこの
サーバーで自動整形されるようになる。停止すると文字起こし結果はそのまま(未整形)で返る。

...
# これだけはコンテナ経由ではなくホストマシン上で直接実行する。
# ---------------------------------------------------------------------------
.PHONY: llm llm-thinking

llm: ## ローカルLLMサーバー(通常/Instructモード)をセットアップ・起動 (ホスト上で直接実行、要Apple Silicon Mac)
	cd apps/llm-server && ./run.sh instruct

llm-thinking: ## ローカルLLMサーバー(シンキングモード)をセットアップ・起動 (ホスト上で直接実行、要Apple Silicon Mac)
	cd apps/llm-server && ./run.sh thinking
